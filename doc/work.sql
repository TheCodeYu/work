...
create table sys_user (
  ...
  user_type         varchar(10)     not null                   comment '用户类型（00系统用户）',  
  -- user_type由产品注册方填写system，即产品标志符
...
-- 初始化-用户信息表数据
-- ----------------------------
insert into sys_user values(1,  103, 'admin', '超级管理员', '00', 'ry@163.com', '15888888888', '1', '', '$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu2', '0', '0', '127.0.0.1', sysdate(), 'admin', sysdate(), '', null, '管理员');
insert into sys_user values(2,  105, 'test',    '测试员', '00', 'ry@qq.com',  '15666666666', '1', '', '$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu2', '0', '0', '127.0.0.1', sysdate(), 'admin', sysdate(), '', null, '测试员');
...
insert into sys_menu values('4', '若依官网', '0', '4', 'http://4mychip.com', null ,  0, 0, 'M', '0', '0', '', 'guide',  'admin', sysdate(), '', null, '官网地址');
-- 二级菜单
...
create table sys_show (
  show_id           int(4)          not null auto_increment    comment '展示ID',
  show_title        varchar(50)     not null                   comment '展示标题',
  show_type         char(1)         not null                   comment '展示类型（1通知 2公告）',
  show_content      varchar(255)     default null              comment '展示摘要',
  status            char(1)         default '0'                comment '展示状态（0正常 1关闭）',
  create_by         varchar(64)     default ''                 comment '创建者',
  create_time       datetime                                   comment '创建时间',
  update_by         varchar(64)     default ''                 comment '更新者',
  update_time       datetime                                   comment '更新时间',
  remark            varchar(255)    default null               comment '备注',
  primary key (show_id)
) engine=innodb auto_increment=10 comment = '前台展示表';
...
create table sys_product (
  product_id        int(4)          not null auto_increment    comment '产品ID',
  product_name      varchar(50)     not null                   comment '产品名称',
  product_type      char(1)         not null                   comment '产品类型',
  product_content   varchar(255)    default null               comment '产品简介',
  url               varchar(255)    default null               comment '产品图片地址',
  product_url       varchar(255)    default null               comment '下载链接（null非下载产品）',
  product_price     int(4)          default 0                  comment '产品价格（0免费）',
  status            char(1)         default '0'                comment '产品状态（0正常 1关闭）',
  create_by         varchar(64)     default ''                 comment '创建者',
  create_time       datetime                                   comment '创建时间',
  update_by         varchar(64)     default ''                 comment '更新者',
  update_time       datetime                                   comment '更新时间',
...
) engine=innodb auto_increment=10 comment = '产品表';
...
create table sys_pro (
  product_id        int(4)          not null auto_increment    comment '支付ID',
  product_id        varchar(50)     not null                   comment '产品ID',
  product_type      char(1)         default null               comment '支付类型',
  product_content   varchar(255)    default null               comment '支付备注',
  product_price     int(4)          default 0                  comment '支付价格（0免费）',
  status            char(1)         default '0'                comment '支付状态（0正常 1未完成）',
  ...
  remark            varchar(255)    default null               comment '备注',
  primary key (product_id)
) engine=innodb auto_increment=10 comment = '产品购买表';